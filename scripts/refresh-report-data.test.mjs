...
test("fetchGitHubPrActivity canonicalizes redirected repo aliases before counting PRs", async () => {
  ...
  const prActivity = await fetchGitHubPrActivity("2026-04-01", {
    repoTeamMap: {
      "nepgpe/tfc-driver-lawo-homeapps": "bc",
      "nepgpe/tfc-driver-lawo-homeapps-multiviewer": "bc"
    },
    githubToken: "test-token",
    fetchRepoMetadata: async (repo) => ({
      full_name:
        repo === "nepgpe/tfc-driver-lawo-homeapps-multiviewer"
          ? "nepgpe/tfc-driver-lawo-homeapps"
          : repo
    }),
    fetchRepoPage: async (repo, page) => {
      ...
      return [
        {
          ...
          base: { repo: { full_name: "nepgpe/tfc-driver-lawo-homeapps" } },
          user: { login: "author_nepgroup" }
        }
      ];
    },
  ...
  assert.deepEqual(fetchedRepos, ["nepgpe/tfc-driver-lawo-homeapps"]);
  ...
  assert.deepEqual(
    ...
    [
      {
        uniqueKey: "nepgpe/tfc-driver-lawo-homeapps#44",
        repositoryId: "nepgpe/tfc-driver-lawo-homeapps",
        team: "bc"
      }
    ]
  );

...
(function initDashboardProductCharts() {
  ...
  function TrendSvgChart({ rows, colors, yUpper }) {
    ...
    function showTooltip(point) {
      const pointDateLabel = String(point?.dateShort || point?.label || point?.date || "").trim();
      setTooltipContent(
        h(
          "div",
          null,
          h("p", null, h("strong", null, point.lineDef.name)),
          h("p", null, pointDateLabel),
            ...
            ...(series.dot === false
              ...
              : series.points.map((point) =>
                  h("circle", {
                    ...
                    tabIndex: 0,
                    role: "button",
                    "aria-label": `${point.lineDef.name} on ${
                      point.dateShort || point.label || point.date || ""
                    }: ${toWhole(point.value)} open bugs`,
